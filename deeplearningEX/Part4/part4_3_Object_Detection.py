...
def yolo_filter_boxes(box_confidence, boxes, box_class_probs, threshold=.6):
    ...
    # Step 2: Find box_classes thanks to the max box_scores, keep track of the corresponding score
    box_classes = K.argmax(box_scores, axis=-1)
    box_class_scores = K.max(box_scores, axis=-1, keepdims=False)

    # Step 3: Creating a filtering mask based on "box_class_scores" by using threshold.
    #       The mask should have the same dimension as box_class_scores, and be True
    #       for the boxes you want to keep
    filtering_mask = box_class_scores >= threshold
    # (19,19,5,1)

    # Step 4: Apply the mask to scores, boxes and classes
    # mask维度(19,19,5,1)从前往后匹配，最后一维是多少不关心，前三位匹配，保留满足的boxes
    scores = tf.boolean_mask(box_class_scores, filtering_mask)  #(19,19,5,1)
    boxes = tf.boolean_mask(boxes, filtering_mask)              #(19,19,5,4)
    classes = tf.boolean_mask(box_classes, filtering_mask)      #(19,191,5,1)
